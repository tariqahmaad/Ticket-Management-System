...
def login_security(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(
                request, "security_login.html", {"error_message": "Invalid login"}
            )
        ...
        if user is not None:
            if user.is_active:
                if user.user_type == 1:
                    login(request, user)
                    return redirect(
                        "staff_home", flight_no=user.staff.flight_no.flight_no
                    )
                else:
                    return render(
                        request,
            ...
            return render(
                request, "staff_login.html", {"error_message": "Invalid login"}
            )
...
def clear_for_security(request, pk):
    ...
    try:
        security_id = request.user.security.id
    except User.security.RelatedObjectDoesNotExist:
        return HttpResponse("User has no associated security object.", status=400)

    passenger.cleared_security_status = security_id
    passenger.save()
...
def staff_home(request, flight_no):
    ...
    return render(
        request, "staff_home.html", {"passengers": data, "flight_no": flight_no}
    )
...
def book_flight(request, pk):
    if request.method == "POST":
        ...
        passenger.save()
        passenger.pnr = (
            str(flight.flight_no) + str(flight.destination_code) + str(passenger.pk)
        )
        passenger.save()
...
def view_booking(request):
    if request.method == "POST":  # view existing booking
        ...
        if passenger:
            ...
            return render(
                request, "home.html", {"error_message_booking": "No booking found"}
            )
...
def airport_mgmt(request):
    ...
    return render(request, "airport_mgmt.html", {"flight_count": flight_count})
